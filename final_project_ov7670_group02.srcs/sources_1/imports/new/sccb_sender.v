...
(
    ...
    // Main I2C Transfer State Machine
    always @(posedge iCLK or negedge iRST_N) 
    begin
        if(!iRST_N) 
            begin 
            ...
            if(GO)
                begin
                if(WR) // Case 1: I2C Write Sequence
                    begin
                    case(SD_COUNTER)
                    6'd0 : begin SCLK <= 1; I2C_BIT <= 1; ACKW1 <= 1; ACKW2 <= 1; ACKW3 <= 1; END <= 0; end
                    6'd1 : begin SCLK <= 1; I2C_BIT <= 1; END <= 0; end
                    6'd2 : I2C_BIT <= 0; // START Condition
                    6'd3 : SCLK <= 0;
                    
                    // SLAVE ADDR (8 bits) + ACK1
                    ...
                    begin
                    case(SD_COUNTER)
                    6'd0 : begin SCLK <= 1; I2C_BIT <= 1; ACKW1 <= 1; ACKR1 <= 1; END <= 0; end
